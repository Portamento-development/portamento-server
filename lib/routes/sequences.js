...
router
    ...
    // `.get('/')` already does this. I would have added a `limit` query param to that
    // route rather than adding this one
    .get('/by_patch/:patchId', (req, res, next) => {
        Sequence.findOne({'patchId': req.params.patchId})
            .populate('patchId')
            .lean()
            .then(sequence => res.send(sequence))
            .catch(next);
